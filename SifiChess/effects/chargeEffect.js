import { Canvas } from "../common/tools.js";
import { ExpColor as SLDC } from "../common/const.js";

export class ChargeEffect {
  constructor(attacker, defender, _cxt) {
    this.cxt = _cxt;
    this.x = defender.x;
    this.y = defender.y;
    this.time = 0;
    this.maxTime = 20;
    this.isAlive = true;

    this.drawCross = function (x0, y0) {
      let len = 10;
      let weight = 8;
      let x1 = x0 - len;
      let y1 = y0;
      let x2 = x0 + len;
      let y2 = y0;
      let x3 = x0;
      let y3 = y0 - len;
      let x4 = x0;
      let y4 = y0 + len;
      Canvas.drawLine(this.cxt, x1, y1, x2, y2, SLDC, weight);
      Canvas.drawLine(this.cxt, x3, y3, x4, y4, SLDC, weight);
    };

    this.drawLine = function () {
      let weight = 6;
      Canvas.drawLine(
        this.cxt,
        attacker.x + 25,
        attacker.y + 25,
        this.x + 25,
        this.y + 25,
        SLDC,
        weight
      );
    };

    this.draw = function () {
      this.time++;
      if (this.time > this.maxTime) {
        this.isAlive = false;
        return;
      }

      let x1 = this.x + 15;
      let x2 = this.x + 35;
      let y1 = this.y + 25 - Math.round(this.time);
      let y2 = this.y + 45 - Math.round(this.time);

      this.drawCross(x1, y1);
      this.drawCross(x2, y2);
      this.drawLine();
    };
  }
}
