...
var Info = {
        ...
        if (T.pointInRect({ x: x, y: y }, self.towerPosition[i])) {
          ...
          select.onmousemove = function (e) {
            ...
            if (
              ...
              !self.installTower[xIndex + "_" + yIndex]
            )
              Canvas.fillArc(
                cxt,
                x,
                y,
                TowerType[i]["level_1"].scope,
                "rgba(25,174,70,0.5)"
              );
            else
              Canvas.fillArc(
  ...
  drawScope: function (tower) {
    ...
    Canvas.fillArc(
      ...
      tower.x + 25,
      tower.y + 25,
      TowerType[tower.type]["level_" + tower.level].scope,
      "rgba(25,174,70,0.5)"
    );
  ...
  upgradeOrSell: function (x, y) {
    ...
    if (
      ...
      T.pointInRect(
        { x: x, y: y },
        { x: tower.x, y: tower.y, width: 20, height: 20 }
      )
    ) {
      if (
        this.score - TowerType[tower.type]["level_" + (tower.level + 1)].buyIt <
        0
      )
        return false;
      ...
      this.updateScore(
        TowerType[tower.type]["level_" + tower.level].buyIt * -1
      );
    ...
    //卖掉
      ...
      T.pointInRect(
        { x: x, y: y },
        { x: tower.x + 30, y: tower.y + 30, width: 20, height: 20 }
      )
    ) {
      var money = Math.floor(
        (tower.level * TowerType[tower.type]["level_1"].buyIt) / 2
      );

      this.updateScore(money);

      delete this.installTower[
        Math.floor(tower.x / 50) + "_" + Math.floor(tower.y / 50)
      ];

      Game.towerList.remove(tower);

      Canvas.clearRect(
        Game.canvasList.tower,
        tower.x,
        tower.y,
        tower.width,
        tower.height
